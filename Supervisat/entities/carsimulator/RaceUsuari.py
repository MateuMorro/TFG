...
from entities.carsimulator.CarUsuari import CarUsuari
...
class RaceUsuari(object):
    def __init__(self,n,k,m, net):
        self._track = Track(n)
        self._cars = []
        self.__n=n
        self.__net = net
    ...
    def get_first_car(self) -> CarUsuari:
        max_distance = None
        first_car = None
        for c in self._cars:
    ...
    def emulate(self,x,y,angle,tecla):
        #self.__total_time = self.__total_time + elapsed_time
        self._cars[0].colocar(x,y,angle)
        sensors=self._cars[0].compute_collision_points()

        for k in range(1, self.__number_cars):
            c=self._cars[k]
