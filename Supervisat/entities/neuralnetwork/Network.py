...
class Network(object):
    ...
    def feedforward(self, a):
        ...
        for b, w in zip(self.biases, self.weights):
            ...
            #tanHip
            #a = Network.tanh(numpy.dot(w, a) + b)
            #a = Network.arctan(numpy.dot(w, a) + b)

            # softmax
            #a = Network.softmax(numpy.dot(w, a) + b)

            #a = Network.tangenteHiperbolica(numpy.dot(w, a) + b)
            # Funció radial hipesferico
            #a = Network.gaussian(numpy.sqrt(numpy.sum([numpy.power(numpy.dot(w,a),2.0)], axis=0)) + b,1)
